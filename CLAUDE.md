...
### Auth
- **Vendeurs** : Phone OTP via Supabase Auth + bypass dev pour `+221706152830`
...
## 3. Refonte UI — Design Minimal Notion (TERMINEE)
...
- **Icones** : Texte simple, pas d'emojis nulle part (HTML, JS, sidebar, nav)
...
### Progression de la refonte — TOUT FAIT

| Categorie | Fichiers | Statut |
|-----------|----------|--------|
| Design system | `tailwind.config.js`, `css/input.css`, `css/animations.css` | FAIT |
| Composants JS | `sidebar.js`, `admin-nav.js`, `fullscreen-menu.js`, `theme-manager.js` | FAIT |
| JS utilitaires | `app.js`, `notifications.js`, `geolocation.js` | FAIT |
| Pages publiques | `index.html`, `catalogue.html`, `demo-shop.html`, `panier-demo.html`, `cgu.html` | FAIT |
| Pages auth vendeur | `vendeur/connexion.html`, `vendeur/inscription.html` | FAIT |
| Pages dashboard vendeur | `dashboard`, `produits`, `commandes`, `clients`, `analytics`, `abonnements`, `profil`, `factures`, `equipe`, `multi-boutiques`, `produit-details`, `inventaire`, `index` | FAIT |
| Pages entreprise vendeur | `agents`, `creation-boutique`, `depot`, `transferts`, `comptabilite`, `rapports` | FAIT |
| Pages admin | Toutes (connexion, dashboard, admins, vendeur, stores, analytics, subscriptions, commercial, gestionnaire, financial) | FAIT |
| Build CSS | `output.css` genere et minifie | FAIT |
...
- **Effets visuels** : glassmorphism, backdrop-blur, hover-lift, hover-scale, hover-glow, card-premium, btn-premium, modal-premium, shadow-premium, gradient-overlay, relief effects — tous supprimes
- **Couleurs anciennes** : #667eea, #25D366, #128C7E, #f7fafc — toutes remplacees par la palette Notion
- **Emojis** : Supprimes de TOUTES les pages HTML et fichiers JS (sidebar, nav, titres, boutons, etats vides, factures)
- **Inline styles anciens** : `<style>` blocks dans factures.html, equipe.html, multi-boutiques.html — supprimes ou nettoyes
...
## 7. Nettoyage auth.js (FAIT)

- ~20 debug `fetch('http://127.0.0.1:7242/...')` supprimes (logs agent debug)
- Bypass dev pour inscription : numero `+221706152830` cree user + store directement dans Supabase sans OTP SMS
- Bypass dev pour connexion : connexion directe sans OTP (existait deja, conserve)
- `inscription.html` adapte pour gerer le `bypassOTP` retourne par `inscrireVendeur()`
- Fonction `isDevNumber()` centralisee pour verifier le numero dev
- `DEV_PHONE = '+221706152830'` constant dans auth.js

## 8. Historique de Maintenance

### 2026-02-22 : Restauration Supabase
- **Problème** : Connexion impossible, erreurs de timeout.
- **Cause** : Projet Supabase (`ptscvapqhsctosjpdbkr`) mis en pause par inactivité.
- **Résolution** : Restauration manuelle du projet via les outils d'administration.
- **Vérification** : Tables `users` et `stores` intactes, accès rétabli pour le numéro de test `+221706152830`.

---

*Derniere mise a jour : 2026-02-22 — Projet Supabase restaure + Integration walkthrough*
...
**Le projet ne répond pas (Timeout / Error 500) :**
- Vérifiez si le projet est mis en pause par Supabase (inactivité).
- Allez sur le Dashboard Supabase et cliquez sur **Restore Project**.
- Attendez environ 2-5 minutes pour que le statut passe à `ACTIVE`.

**"relation users does not exist" :**
- Le schéma SQL n'a pas été exécuté. Exécutez le script SQL de schéma complet.
